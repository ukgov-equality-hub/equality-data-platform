...
//  version_label       = aws_elastic_beanstalk_application_version.main_app_elastic_beanstalk_application_version__sample_app.name
  ...
  setting {
    ...
    value     = aws_iam_instance_profile.main_app_iam_instance_profile_eb.name
  }
  ...
  setting {
    ...
    value     = "true"  // disabled. we create our own port 80 listener which redirects to https
  }
...
//  }
  ...
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "ENTERPRISE_TASKFORCE_2_PASSWORD"
    value     = var.ENTERPRISE_TASKFORCE_2_PASSWORD
  }
}
